...
- If you didn't already do this (when we made the link in previous section), setup your working direcotry (in our home space) first. We could actually do this just in the script, but it's not a bad idea to set this up manually then just define it in the script to ensure your job runs at first.
...
```bash
WRKDIR=<Full_Path_To_Your_Home>/BioinfWorkshop2021/Part3_R_RNAseq/
```
...
#SBATCH -o <Full_Path_To_Your_Home>/BioinfWorkshop2021/Part3_R_RNAseq/jobs/PreProcess_RNAseq.outerror
```
...
```bash
sbatch <Full_Path_To_Your_Home>/BioinfWorkshop2021/Part3_R_RNAseq/jobs/PreProcess_RNAseq.sh
```
